...
DATA_PATH = f'{ROOT_DIR}/data_pred'
...
def set_train_val_test_split(
  ...
  train_idx = []
  rnd_state = np.random.RandomState(seed)
  # 每个类选出来20个作为train_idx
  for c in range(data.y.max() + 1):
    class_idx = development_idx[np.where(data.y[development_idx].cpu() == c)[0]]
    train_idx.extend(rnd_state.choice(class_idx, num_per_class, replace=False))
  # development_idx中剩下的做验证集
  ...
  data.train_mask = get_mask(train_idx)  # 140训练集，每类别20个样本
  data.val_mask = get_mask(val_idx)  # 1500-140=1360作为验证集
  data.test_mask = get_mask(test_idx)  # 最大连通图中2485-1500=985个样本作为测试集
