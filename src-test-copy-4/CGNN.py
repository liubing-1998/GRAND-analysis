...
to use pytorch geometric data_pred pipelines
"""
...
def train_ray(opt, checkpoint_dir=None, data_dir='../data_pred', opt_val=False):
  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
def train_ray_icml(opt, checkpoint_dir=None, data_dir="../data_pred", opt_val=False):
  device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
  dataset = get_dataset(opt, data_dir, False)
