...
def get_two_hop(data):
  print('raw data_pred contains {} edges and {} nodes'.format(data.num_edges, data.num_nodes))
  th = TwoHop()
  data = th(data)
  print('following rewiring data_pred contains {} edges and {} nodes'.format(data.num_edges, data.num_nodes))
  return data
...
def KNN(x, opt):
  ...
  # distance between all the grid points and all the random data_pred points
...
def apply_beltrami(data, opt, data_dir=f'{ROOT_DIR}/data_pred'):
  ...
  if os.path.exists(fname):
    ...
    if opt['pos_enc_type'].startswith("DW"):
      pos_encoding = pos_encoding['data_pred']
...
def apply_pos_dist_rewire(data, opt, data_dir='../data_pred'):
