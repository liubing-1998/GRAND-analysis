...
def load_data(opt):
  ...
  if opt['im_dataset'] == 'MNIST':
    ...
    data_train = torchvision.datasets.MNIST('../data_pred/'+exdataset + '/', train=True, download=True,
                                            transform=transform)
    data_test = torchvision.datasets.MNIST('../data_pred/'+exdataset+'/', train=False, download=True,
                                           transform=transform)
  elif opt['im_dataset'] == 'CIFAR':
    transform = transforms.Compose([transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    data_train = torchvision.datasets.CIFAR10('../data_pred/' + exdataset + '/', train=True, download=True,
                                            transform=transform)
    data_test = torchvision.datasets.CIFAR10('../data_pred/' + exdataset + '/', train=False, download=True,
                                           transform=transform)
  ...
  if opt['testing_code'] == True:
      Graph_GNN = create_in_memory_dataset(opt, "GNN", train_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+'GNN/', processed_file_name='Graph'+exdataset+'GNN.pt')
      Graph_train = create_in_memory_dataset(opt, "Train", train_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+ str(opt['train_size'])+'Train/', processed_file_name='Graph'+exdataset+str(opt['train_size']) +'Train.pt')
      Graph_test = create_in_memory_dataset(opt, "Test", test_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+ str(opt['test_size'])+'Test/', processed_file_name='Graph'+exdataset+str(opt['test_size']) +'Test.pt')
  else:
      Graph_GNN = create_in_memory_dataset(opt, "GNN", train_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+'GNN/', processed_file_name='PyG'+exdataset+'GNN.pt')
      Graph_train = create_in_memory_dataset(opt, "Train", train_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+'Train/', processed_file_name='PyG'+exdataset+'Train.pt')
      Graph_test = create_in_memory_dataset(opt, "Test", test_loader, edge_index, im_height, im_width, im_chan,
                                             root='../data_pred/PyG'+exdataset+'Test/', processed_file_name='PyG'+exdataset+'Test.pt')
...
def load_Superpix75Mat(opt): #, path='../data_pred/SuperMNIST/MNIST/datasets/mnist_superpixels_data_75/'):
    ...
    #             root='../data_pred/SuperPix75'+'_'+type+'/', processed_file_name='GraphSuperPix75'+type+'.pt')
    type = "Train"
    Graph_train = create_Superpix75(opt, type,
                root='../data_pred/SuperPix75'+ str(opt['train_size']) +type+'/', processed_file_name='GraphSuperPix75'+ str(opt['train_size'])+type+'.pt')
    type = "Test"
    Graph_test = create_Superpix75(opt, type,
                root='../data_pred/SuperPix75'+ str(opt['test_size']) +type+'/', processed_file_name='GraphSuperPix75'+ str(opt['test_size'])+type+'.pt')
...
if __name__ == "__main__":
    ...
    # Cora = get_dataset('Cora', '../data_pred', False)
    ...
    # train_loader = torch.utils.data_pred.DataLoader(
    #   torchvision.datasets.MNIST('data_pred/' + exdataset + '/', train=True, download=True,
    ...
    #
    # train_loader = torch.utils.data_pred.DataLoader(
    #   torchvision.datasets.CIFAR10('data_pred/' + exdataset + '/', train=True, download=True,
    #                              transform=transform),
    #                                 batch_size=1, shuffle=True)
    #
    #
    ...
    #                                                                         root='./data_pred/Graph' + exdataset + 'GNN/',
