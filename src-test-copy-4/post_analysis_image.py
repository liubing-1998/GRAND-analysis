...
def print_image_path(model, dataset, opt, height, width, frames):
  ...
  # loop through data_pred and update plot
  def update(ii):
    for i in range(height * width):
      plt.subplot(height, width, i + 1)
      plt.tight_layout()
...
def main(opt):
  ...
  for batch_idx, batch in enumerate(loader):
    if batch_idx == 0:# only do this for 1st batch/epoch
      # model.data_pred = batch #loader.dataset  #adding this to reset the data_pred
      # model.odeblock.data_pred = batch #loader.dataset.data_pred #why do I need to do this? duplicating data_pred from model to ODE block?
      # model.odeblock.odefunc.adj = get_rw_adj(model.data_pred.edge_index) #to reset adj matrix
      break
